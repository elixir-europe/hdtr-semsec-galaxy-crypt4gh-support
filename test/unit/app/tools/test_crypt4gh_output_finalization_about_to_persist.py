...
def test_finalize_about_to_persist_payload_fails_when_extra_files_encrypted_target_already_exists(tmp_path, monkeypatch):
    output_path = tmp_path / "discover" / "sample1.tsv"
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text("sample\n")
    ...
    (extra_files_root / "foo.txt").write_text("foo\n")
    (extra_files_root / "foo.txt.c4gh").write_text("already-encrypted\n")
    ...
    with pytest.raises(Crypt4GHRemoteExecutionError, match="encrypted payload already exists"):
        finalize_about_to_persist_crypt4gh_payload(
            output_path=str(output_path),
            dataset_output_path=str(tmp_path / "objects" / "dataset_1.dat"),
            plaintext_path=str(tmp_path / "_crypt" / "outputs" / "ds_1" / "plaintext"),
...
def test_finalize_about_to_persist_payload_rejects_discovered_mapping_path_outside_allowed_roots(tmp_path, monkeypatch):
    output_path = tmp_path / "discover" / "sample1.tsv"
    output_path.parent.mkdir(parents=True, exist_ok=True)
    output_path.write_text("sample\n")

    marker_dir = tmp_path / "_c4gh_stage" / "outputs"
    outside_mapping_path = tmp_path / "outside" / "discovered_designations.json"
    outside_mapping_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    with pytest.raises(Crypt4GHRemoteExecutionError, match="outside allowed roots"):
        finalize_about_to_persist_crypt4gh_payload(
            ...
            encrypted_marker_path=str(marker_dir / "ds_1.encrypted"),
            designation="sample1",
            discovered_marker_map_path=str(outside_mapping_path),
            allowed_root_paths=[str((tmp_path / "discover").resolve())],
        )
