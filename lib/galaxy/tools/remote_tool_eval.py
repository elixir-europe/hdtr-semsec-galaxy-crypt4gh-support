...
def _crypt4gh_finalize_postrun_command(
    *,
    ...
    finalize_script = (
        ...
        "    return False\n"
        "\n"
        "def _safe_remove(path: str, roots, stats) -> None:\n"
        "    if not path:\n"
        "        return\n"
        "    if not _is_within_allowed_roots(path, roots):\n"
        "        stats['outside_allowed_roots'] += 1\n"
        ...
        "            stats['concurrent_mutation'] += 1\n"
        "            print(f'Crypt4GH best-effort purge observed concurrent mutation for {path} ({exc.__class__.__name__}: {exc})', file=sys.stderr)\n"
        "        except Exception as exc:\n"
        "            stats['failed'] += 1\n"
        "            print(f'Crypt4GH best-effort purge failed for {path} ({exc.__class__.__name__}: {exc})', file=sys.stderr)\n"
        "\n"
        "def _purge_plaintext_payloads(targets):\n"
        "    stats = {\n"
        "        'removed': 0,\n"
        "        'missing': 0,\n"
        "        'outside_allowed_roots': 0,\n"
        "        'concurrent_mutation': 0,\n"
        "        'failed': 0,\n"
        "    }\n"
        ...
        "        _safe_remove(str(target.get('output_path', '') or ''), _roots, stats)\n"
        "        _safe_remove(str(target.get('dataset_output_path', '') or ''), _roots, stats)\n"
        "        _safe_remove(str(target.get('extra_files_output_path', '') or ''), _roots, stats)\n"
        "        _safe_remove(str(target.get('encrypted_marker_path', '') or ''), _roots, stats)\n"
        "        _safe_remove(str(target.get('extra_files_manifest_path', '') or ''), _roots, stats)\n"
        "    if stats['concurrent_mutation'] or stats['failed'] or stats['outside_allowed_roots']:\n"
        "        print(\n"
        "            'Crypt4GH best-effort purge partial outcome: '\n"
        "            f\"removed={stats['removed']} missing={stats['missing']} \"\n"
        "            f\"outside_allowed_roots={stats['outside_allowed_roots']} \"\n"
        "            f\"concurrent_mutation={stats['concurrent_mutation']} failed={stats['failed']}\",\n"
        "            file=sys.stderr,\n"
        "        )\n"
        "\n"
        "try:\n"
